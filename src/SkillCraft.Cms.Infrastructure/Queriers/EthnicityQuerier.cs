...
internal class EthnicityQuerier : IEthnicityQuerier
{
  private readonly IActorService _actorService;
  private readonly DbSet<LineageEntity> _lineages;
  private readonly ISqlHelper _sqlHelper;

  public EthnicityQuerier(IActorService actorService, RulesContext rules, ISqlHelper sqlHelper)
  {
  ...
  public async Task<EthnicityModel?> ReadAsync(Guid id, CancellationToken cancellationToken)
  {
  ...
  public async Task<SearchResults<EthnicityModel>> SearchAsync(SearchEthnicitiesPayload payload, CancellationToken cancellationToken)
  {
    ...
    IReadOnlyCollection<EthnicityModel> ethnicities = await MapAsync(entities, cancellationToken);

    return new SearchResults<EthnicityModel>(ethnicities, total);
  }

  private async Task<EthnicityModel> MapAsync(LineageEntity lineage, CancellationToken cancellationToken)
  {
    return (await MapAsync([lineage], cancellationToken)).Single();
  }
  private async Task<IReadOnlyCollection<EthnicityModel>> MapAsync(IEnumerable<LineageEntity> lineages, CancellationToken cancellationToken)
  {
    ...
    return lineages.Select(mapper.ToEthnicity).ToList().AsReadOnly();
  }
}
